...
const getCurrentPosition = () => new Promise((resolve, reject) => {
  ...
  const handleError = (error) => {
    const messages = {
      ...
      2: "Your location could not be determined. Please turn on Windows/browser location services and try again.",
      3: "Location request timed out. Please wait a few seconds and try again.",
    };
...
const getStableLiveLocation = async () => {
  // Desktop browser location can occasionally jump to a stale/network-derived
  // position. Take a few fresh readings and prefer the most accurate one.
  const readings = [];
  let lastError = null;
  for (let i = 0; i < 3; i += 1) {
    try {
      ...
      if (Number.isFinite(latitude) && Number.isFinite(longitude)) {
        readings.push({ latitude, longitude, accuracy: Number.isFinite(accuracy) ? accuracy : null });
      }
  ...
  if (!readings.length) throw lastError || new Error("Unable to read your live location.");
  readings.sort((a, b) => (a.accuracy ?? Number.POSITIVE_INFINITY) - (b.accuracy ?? Number.POSITIVE_INFINITY));
  return readings[0];
};

export const getLiveLocation = async () => {
  const position = await getStableLiveLocation();
  return {
...
export const getAttendanceLocationStatus = async () => {
  const policy = await getAttendancePolicy();
  const officeLatitude = Number(policy.officeLatitude);
  const officeLongitude = Number(policy.officeLongitude);
  const radiusMeters = Math.max(25, Number(policy.officeRadiusMeters) || 200);
  const location = await getLiveLocation();
  const distanceMeters = Number.isFinite(officeLatitude) && Number.isFinite(officeLongitude)
    ? distanceInMeters(officeLatitude, officeLongitude, location.latitude, location.longitude)
    : null;
  return {
    officeLatitude: Number.isFinite(officeLatitude) ? officeLatitude : null,
    officeLongitude: Number.isFinite(officeLongitude) ? officeLongitude : null,
    radiusMeters,
    ...location,
