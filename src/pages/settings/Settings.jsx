...
import { getLiveLocation, getAttendanceLocationStatus } from "../../services/attendanceService";
...
function Settings() {
  ...
  */
  ...
  const testCurrentAttendanceLocation = async () => {
    try {
      setLocationDetecting(true);
      setAttendancePolicyError("");
      ...
      setAttendancePolicyError(err?.message || "Unable to test the current location.");
    } finally {
      setLocationDetecting(false);
    }
  };
        ...
        >
          ...
          <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50/70 p-4 dark:border-blue-900/40 dark:bg-blue-950/20">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <button type="button" onClick={testCurrentAttendanceLocation} disabled={locationDetecting} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">
                <LocateFixed size={16} /> Test location now
              </button>
            </div>
          </div>
          {locationTest && (
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
              <div className="grid gap-3 md:grid-cols-3">
                <div><p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Detected now</p><p className="mt-1 text-xs font-medium text-slate-900 dark:text-white">{locationTest.latitude.toFixed(7)}, {locationTest.longitude.toFixed(7)}</p></div>
                <div><p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">GPS accuracy</p><p className="mt-1 text-sm font-bold text-slate-900 dark:text-white">±{locationTest.accuracy ? Math.round(locationTest.accuracy) : "—"}m</p></div>
                <div><p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Distance from saved office</p><p className="mt-1 text-sm font-bold text-slate-900 dark:text-white">{locationTest.distanceMeters === null ? "Office location not saved yet" : `${locationTest.distanceMeters}m / ${locationTest.radiusMeters}m`}</p></div>
              </div>
            </div>
          )}
