  ...
  */
  ...
  const testLiveLocation = async () => {
    try {
      ...
      setError("");
      const result = await getAttendanceLocationStatus();
      setLocationStatus(result);
    } catch (err) {
      setError(err?.message || "Unable to test your live location.");
    } finally {
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="mb-5 rounded-2xl border border-blue-100 bg-blue-50/70 p-4 dark:border-blue-900/40 dark:bg-blue-950/20">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-white">Live location check</p>
              <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">Test exactly what this browser reports before Check In. This helps detect incorrect desktop/network location.</p>
            </div>
            <button type="button" onClick={testLiveLocation} disabled={locationChecking} className="inline-flex items-center justify-center gap-2 rounded-xl border border-blue-200 bg-white px-4 py-2.5 text-sm font-semibold text-blue-700 hover:bg-blue-50 disabled:opacity-60 dark:border-blue-800 dark:bg-slate-900 dark:text-blue-300">
              {locationChecking ? <Loader2 size={16} className="animate-spin" /> : <RefreshCw size={16} />}
              {locationChecking ? "Checking location..." : "Test live location"}
            </button>
          </div>
          {locationStatus && (
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-xl bg-white/80 p-3 dark:bg-slate-900/70"><p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Your location</p><p className="mt-1 text-xs font-medium text-slate-900 dark:text-white">{locationStatus.latitude.toFixed(7)}, {locationStatus.longitude.toFixed(7)}</p></div>
              <div className="rounded-xl bg-white/80 p-3 dark:bg-slate-900/70"><p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Office location</p><p className="mt-1 text-xs font-medium text-slate-900 dark:text-white">{locationStatus.officeLatitude !== null ? `${locationStatus.officeLatitude.toFixed(7)}, ${locationStatus.officeLongitude.toFixed(7)}` : "Not configured"}</p></div>
              <div className="rounded-xl bg-white/80 p-3 dark:bg-slate-900/70"><p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Accuracy</p><p className="mt-1 text-sm font-bold text-slate-900 dark:text-white">±{locationStatus.accuracy ? Math.round(locationStatus.accuracy) : "—"}m</p></div>
              <div className="rounded-xl bg-white/80 p-3 dark:bg-slate-900/70"><p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Distance</p><p className="mt-1 text-sm font-bold text-slate-900 dark:text-white">{locationStatus.distanceMeters !== null ? `${locationStatus.distanceMeters}m / ${locationStatus.radiusMeters}m` : "—"}</p></div>
            </div>
          )}
          {locationStatus?.distanceMeters !== null && locationStatus?.distanceMeters > locationStatus?.radiusMeters && (
            <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-300">The browser is currently reporting a location outside the configured office radius. Fix the device/browser location source before Check In.</p>
          )}
        </div>
